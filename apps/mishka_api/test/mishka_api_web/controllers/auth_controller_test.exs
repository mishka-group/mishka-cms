defmodule MishkaApiWeb.AuthControllerTest do
  ...
  describe "Happy | MishkaApi Auth Controller (▰˘◡˘▰)" do
    ...
    test "Change Password", %{conn: conn} do
      ...
      conn =
        ...
        |> post(Routes.auth_path(conn, :change_password), %{
          curent_password: @user_info.password,
          new_password: "Test123Abs"
        })
    ...
    test "Delete Tokens by Email Link send request", %{conn: conn} do
      ...
      conn =
        conn
        |> post(Routes.auth_path(conn, :send_delete_tokens_link_by_email), %{
          email: @user_info.email
        })
  ...
  describe "UnHappy | MishkaApi Auth Controller ಠ╭╮ಠ" do
    test "Register with password", %{conn: conn} do
      without_user_name =
        Map.drop(@user_info, [:password]) |> Map.merge(%{full_name: 1, username: 1})
    ...
    test "Login", %{conn: conn} do
      ...
      conn1 =
        post(conn, Routes.auth_path(conn, :login), %{email: @user_info.email, password: "test"})

      conn2 =
        post(conn, Routes.auth_path(conn, :login), %{username: "username", password: "test"})
    ...
    test "Logout", %{conn: conn} do
      ...
      ref_acc_token =
        MishkaUser.Token.Token.create_token(user_fake, MishkaApi.get_config(:token_type))
    ...
    test "Change Password", %{conn: conn} do
      ...
      conn2 =
        ...
        |> put_req_header("authorization", "Bearer #{auth["access_token"]}")
        |> post(Routes.auth_path(conn, :change_password), %{
          curent_password: "tEstthis123",
          new_password: "tEstthis1235"
        })
    ...
    test "Get Token Expire Time", %{conn: conn} do
      ...
      conn =
        conn
        |> put_req_header("authorization", "Bearer #{ref_acc_token.access_token.token}")
        |> post(Routes.auth_path(conn, :get_token_expire_time), %{
          token: ref_acc_token.access_token.token
        })
