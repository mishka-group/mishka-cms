defmodule MishkaContentTest.BookmarkTest do
  ...
  describe "Happy | Bookmark CRUD DB (▰˘◡˘▰)" do
    test "create a bookmark", context do
      {:ok, :add, :bookmark, _bookmark_info} =
        assert Bookmark.create(Map.merge(@bookmark_info, %{user_id: context.user_info.id}))
    end
    ...
    test "edit a bookmark", context do
      ...
      {:ok, :edit, :bookmark, _bookmark_info} =
        assert Bookmark.edit(
                 Map.merge(@bookmark_info, %{
                   id: bookmark_info.id,
                   extra: %{test: "test bookmark 2"}
                 })
               )
    end
    ...
    test "show by id", context do
      ...
      {:ok, :get_record_by_id, :bookmark, _bookmark_info} =
        assert Bookmark.show_by_id(bookmark_info.id)
    end
    ...
    test "show user Bookmark", context do
      ...
      1 =
        assert length(
                 Bookmark.bookmarks(
                   conditions: {1, 10},
                   filters: %{user_id: bookmark_info.user_id}
                 ).entries
               )
      ...
      1 =
        assert length(
                 Bookmark.bookmarks(
                   conditions: {1, 10},
                   filters: %{user_id: bookmark_info.user_id, status: :active}
                 ).entries
               )
    end
  end
  ...
  describe "UnHappy | Bookmark CRUD DB ಠ╭╮ಠ" do
    ...
    test "create a bookmark", context do
      {:error, :add, :bookmark, _bookmark_info} =
        assert Bookmark.create(%{user_id: context.user_info.id})
    end
  end
end
