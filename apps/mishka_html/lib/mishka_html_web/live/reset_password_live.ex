defmodule MishkaHtmlWeb.ResetPasswordLive do
  ...
  def handle_params(%{"random_link" => random_link}, _url, socket) do
    ...
    socket =
      with {:random_code, false, random_code_info} <-
             {:random_code, is_nil(random_code), random_code},
           {:ok, :get_record_by_field, :user, _repo_data} <-
             MishkaUser.User.show_by_email(random_code_info.email) do
        socket
        |> put_flash(
          :success,
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "کد ارسالی از طرف شما صحیح می باشد. لطفا پسورد جدید خود را وارد کنید."
          )
        )
        |> push_redirect(
          to: Routes.live_path(socket, MishkaHtmlWeb.ResetChangePasswordLive, random_link)
        )
      else
        {:random_code, true, _random_code_info} ->
          ...
          |> put_flash(
            ...
            MishkaTranslator.Gettext.dgettext(
              "html_live",
              "کد ارسالی شما اشتباه می باشد. لطفا دوباره تلاش کنید"
            )
          )
        ...
        {:error, :get_record_by_field, _error_tag} ->
          ...
          |> put_flash(
            ...
            MishkaTranslator.Gettext.dgettext(
              "html_live",
              "چنین کاربری وجود ندارد یا حذف شده است."
            )
          )
          |> push_redirect(to: Routes.live_path(socket, __MODULE__))
      end
    ...
    socket =
      with {:ok, :verify, _token_info} <- MishkaUser.Validation.GoogleRecaptcha.verify(token),
        ...
        MishkaContent.General.Activity.create_activity_by_start_child(
          ...
          %{user_action: "send_reset_password", type: "client", user_id: repo_data.id}
        )
        ...
        description =
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "برای حساب کاربری شما درخواست تغییر پسورد ارسال شده است در صورتی که شما این درخواست را انجام ندادید لطفا این پیام را در نظر نگیرید و در صورت تکرار لطفا پسورد قوی تر و همینطور با بخش پشتیبانی در تماس باشید."
          )
        ...
        |> put_flash(
          ...
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "در صورتی که در بانک اطلاعاتی ما حساب کاربری ای داشته باشید یا در ۵ دقیقه اخیر درخواست فراموشی پسورد ارسال نکرده باشید به زودی برای شما یک ایمیل ارسال خواهد شد. لازم به ذکر است در صورت نبودن ایمیل در اینباکس لطفا محتوای اسپم یا جانک میل را نیز چک فرمایید."
          )
        )
        |> push_redirect(to: Routes.live_path(socket, __MODULE__))
      else
