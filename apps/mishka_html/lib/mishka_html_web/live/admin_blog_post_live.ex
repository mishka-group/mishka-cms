defmodule MishkaHtmlWeb.AdminBlogPostLive do
  ...
  def handle_params(%{"id" => id}, _url, socket) do
    ...
    socket = case Post.show_by_id(id) do
      ...
      {:ok, :get_record_by_id, @error_atom, repo_data} ->
        ...
        posts = Enum.map(all_field, fn field ->
         record = Enum.find(creata_post_state(repo_data), fn post -> post.type == field.type end)
  ...
  defp creata_post_state(repo_data) do
    Map.drop(repo_data, [:inserted_at, :updated_at, :__meta__, :__struct__, :blog_categories, :id, :blog_likes, :blog_tags, :blog_tags_mappers, :blog_authors])
    |> Map.to_list()
  ...
  def basic_menu_list() do
    [
      ...
      description: MishkaTranslator.Gettext.dgettext("html_live", "انتخاب لینک مطلب برای ثبت و نمایش به کاربر. این فیلد یکتا می باشد.")},
      ...
      description: MishkaTranslator.Gettext.dgettext("html_live", "به واسطه این فیلد می توانید مطلب مورد نظر خود را به یک مجموعه تخصیص بدهید")},
  ...
  def more_options_menu_list() do
    [
      ...
      description: MishkaTranslator.Gettext.dgettext("html_live", "شما به واسطه این فیلد می توانید تاریخ انقضا برای یک محتوا را مشخص کنید.")},
      ...
      description: MishkaTranslator.Gettext.dgettext("html_live", "نحوه نمایش مطلب برای مدیریت بهتر دسترسی های کاربران.")},
