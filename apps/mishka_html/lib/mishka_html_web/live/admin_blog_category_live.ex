defmodule MishkaHtmlWeb.AdminBlogCategoryLive do
  ...
  def handle_params(%{"id" => id}, _url, socket) do
    ...
    socket =
      case Category.show_by_id(id) do
        {:error, :get_record_by_id, @error_atom} ->
          ...
          |> put_flash(
            ...
            MishkaTranslator.Gettext.dgettext(
              "html_live",
              "چنین مجموعه ای وجود ندارد یا ممکن است از قبل حذف شده باشد."
            )
          )
        ...
        {:ok, :get_record_by_id, @error_atom, repo_data} ->
          categories =
            Enum.map(all_field, fn field ->
              record =
                Enum.find(creata_category_state(repo_data), fn cat -> cat.type == field.type end)
  ...
  defp create_category(socket,
         params: {params, meta_keywords, main_image, header_image, description, alias_link, sub},
      ...
      case Category.create(
        ...
        {:ok, :add, :category, repo_data} ->
          ...
          Notif.notify_subscribers(%{
            id: repo_data.id,
            msg:
              MishkaTranslator.Gettext.dgettext("html_live", "مجموعه: %{title} درست شده است.",
          ...
          |> put_flash(
            :info,
            MishkaTranslator.Gettext.dgettext("html_live", "مجموعه با موفقیت ایجاد شد")
          )
          |> push_redirect(to: Routes.live_path(socket, MishkaHtmlWeb.AdminBlogCategoriesLive))
      end
  ...
  defp edit_category(socket,
         params:
           {params, meta_keywords, main_image, header_image, description, id, alias_link, sub},
         uploads: {_uploaded_main_image_files, _uploaded_header_image_files}
       ) do
    merge_map =
      %{
      ...
      case Category.edit(Map.merge(merged, exist_images)) do
        ...
        {:ok, :edit, :category, repo_data} ->
          ...
          Notif.notify_subscribers(%{
            id: repo_data.id,
            msg:
              MishkaTranslator.Gettext.dgettext("html_live", "مجموعه: %{title} به روز شده است.",
                title: MishkaHtml.title_sanitize(repo_data.title)
              )
          })
          ...
          |> put_flash(
            :info,
            MishkaTranslator.Gettext.dgettext("html_live", "مجموعه به روز رسانی شد")
          )
          |> push_redirect(to: Routes.live_path(socket, MishkaHtmlWeb.AdminBlogCategoriesLive))
    ...
    [
      ...
      %{
        ...
        description:
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "انتخاب لینک مجموعه برای ثبت و نمایش به کاربر. این فیلد یکتا می باشد."
          )
      },
      %{
        type: "meta_keywords",
    ...
    [
      ...
      %{
        ...
        description:
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "شما می توانید به واسطه این فیلد مجموعه جدید را زیر مجموعه دیگری بکنید"
          )
      },
      %{
        type: "custom_title",
      ...
      %{
        ...
        description:
          MishkaTranslator.Gettext.dgettext(
            "html_live",
            "نحوه نمایش مجموعه برای مدیریت بهتر دسترسی های کاربران."
          )
      },
      %{
      ...
      %{
        ...
        description:
          MishkaTranslator.Gettext.dgettext("html_live", "اجازه ارسال ناتیفیکیشن به کاربران")
      },
      %{
        type: "show_hits",
        status: [
