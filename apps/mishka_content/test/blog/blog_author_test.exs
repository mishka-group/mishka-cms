defmodule MishkaContentTest.Blog.BlogAuthorTest do
  ...
  describe "Happy | Author CRUD DB (▰˘◡˘▰)" do
    ...
    test "edit an author for a post", context do
      {:ok, :add, :blog_author, author_info} =
        assert Author.create(%{
                 "post_id" => context.post_info.id,
                 "user_id" => context.user_info.id
               })
      ...
      {:ok, :edit, :blog_author, _author_info} =
        assert Author.edit(%{
                 id: author_info.id,
                 post_id: context.post_info.id,
    ...
    test "show by id", context do
      ...
      {:ok, :get_record_by_id, :blog_author, _author_info} =
        assert Author.show_by_id(author_info.id)
    end
    ...
    test "show Authors of a post", context do
      ...
      2 =
        assert length(
                 Post.post(context.post_info.alias_link, context.post_info.status).blog_authors
               )
    end
  end
  ...
  describe "UnHappy | Author CRUD DB ಠ╭╮ಠ" do
    ...
    test "create an author for a post", context do
      {:error, :add, :blog_author, _author_info} =
        assert Author.create(%{
