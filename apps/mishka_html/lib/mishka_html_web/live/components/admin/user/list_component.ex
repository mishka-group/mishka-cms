    ...
    ~H"""
      <div class="col bw admin-blog-post-list">
        <div class="table-responsive">
                ...
                <tbody>
                    <%= for {item, color} <- Enum.zip(@users, Stream.cycle(["wlist", "glist"])) do %>
                        <tr class={"blog-list vazir #{if(color == "glist", do: "odd-list-of-blog-posts", else: "")}"}>
                            ...
                            <td class="align-middle text-center">
                                <.live_component module={MishkaHtmlWeb.Public.TimeConverterComponent} id={"inserted-at-#{item.id}-component"} span_id={"inserted-at-#{item.id}-component"} time={item.inserted_at} />
                            </td>
                                ...
                                <div class="col">
                                    ...
                                    <form  phx-change="user_role">
                                        ...
                                        <%= for item <- @roles.entries do %>
                                            <%= if !is_nil(user_role) and item.id == user_role.id do %>
            ...
            <div class="col-sm-10">
                <%= if @users.entries != [] do %>
                <.live_component module={MishkaHtmlWeb.Public.PaginationComponent} id={:pagination} pagination_url={@pagination_url} data={@users} filters={@filters} count={@count} />
                <% end %>
            </div>
        </div>
      </div>
